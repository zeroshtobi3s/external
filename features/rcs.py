...
def RecoilControl_Update(processHandle, clientBaseAddress, Offsets, Options, ARDUINO_HANDLE):
	...
	try:
		...
		if not aimPunchServices:
			return
		aimPunch_x = memfuncs.ProcMemHandler.ReadFloat(processHandle, aimPunchServices + Offsets.offset.m_aimPunchAngle)
		aimPunch_y = memfuncs.ProcMemHandler.ReadFloat(processHandle, aimPunchServices + Offsets.offset.m_aimPunchAngle + 0x4)
